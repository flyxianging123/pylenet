...
if __name__ == "__main__":
    model = cnn_model()
    rootdir = "/home/xiangrong/lx/pycnn/images"  
    for parent,dirnames,filenames in os.walk(rootdir): 
        for filename in filenames:
            print os.path.join(parent,filename)
            im=cv2.imread(os.path.join(parent,filename))
            im_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)   #转换了灰度化
            res=cv2.resize(im_gray,(1,784),interpolation=cv2.INTER_CUBIC)
            t, _ = model.cnn_forward1(res)
            print t
            print np.argmax(t)
